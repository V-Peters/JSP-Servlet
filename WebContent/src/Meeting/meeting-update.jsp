...
<body>
	...
	<form action="MeetingControllerServlet" method=GET>
		...
		<input type="hidden" name="command" value="UPDATEMEETING" />
		...
		<table>
			<tbody>
				<tr>
					...
					<td><input type="text" name="name" value="${MEETING.name}" required maxlength="100" /></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><c:if test="${MEETING.isDisplay()}">
						...
						<c:if test="${not MEETING.isDisplay()}">
				...
				<tr>
					<td><label></label></td>
					<td><input type="submit" value="Anderungen speichern" name="Save" class="save" /></td>
				</tr>
			</tbody>
		</table>
